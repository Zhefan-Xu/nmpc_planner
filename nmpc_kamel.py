	...
	def set_constrains(self):
		...
		for i in range(horizon):
			...
			for obs_pos, size, uncertainty in zip(static_obstacle, static_obstacle_size, static_obstacle_uncertainty):
				# enlarge each obstacle by 3 simga region
				new_size = [size[i] + 3*sqrt(uncertainty[i]) for i in range(2)]
				current_pos = self.pos[:, i]	# current position
				x_diff = current_pos[0] - obs_pos[0]
				y_diff = current_pos[1] - obs_pos[1]

				self.opti.subject_to((x_diff/new_size[0])**2 + (y_diff/new_size[1])**2 >= 1)
